...
function renderInlineMarkdown(text: string): ReactNode[] {
  ...
  while ((match = pattern.exec(text)) !== null) {
    ...
    if (match[2] && match[3]) {
      ...
      nodes.push(
        <strong key={`bold-${key++}`} className="font-semibold text-slate-950 ">
          {match[4]}
        </strong>,
      );
    } else if (match[5]) {
      nodes.push(
        <code
          key={`code-${key++}`}
          className="rounded bg-slate-200 px-1.5 py-0.5 font-mono text-[0.9em] text-slate-900 "
        >
          {match[5]}
        </code>,
      );
      ...
      nodes.push(
        <em key={`italic-${key++}`} className="italic text-slate-900 ">
          {match[6]}
        </em>,
      );
    }
...
function renderMarkdownPreview(content: string): ReactNode[] {
  ...
  if (blocks.length === 0) {
    return [
      <p key="empty" className="text-sm italic text-slate-500 ">
  ...
  return blocks.flatMap((block, index) => {
    if (block.startsWith("# ")) {
      return [
        <h1 key={`h1-${index}`} className="text-2xl font-bold text-slate-950 ">
    ...
    return [
      <p
        key={`p-${index}`}
        className="leading-7 text-slate-700 whitespace-pre-wrap"
      >
...
export function PostForm({ post, authorId }: PostFormProps) {
  ...
  const handleFileImport = async (file: File) => {
    ...
    setUploading(true);
    const modeLabel =
      importMode === "append" ? "chèn vào cuối" : "thay thế nội dung";
    setUploadState({
      name: file.name,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {uploadState && (
        <div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800 /30 ">
      ...
      <div>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          ...
          <label className="inline-flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 sm:w-auto">
            <input
              type="file"
              accept="image/*"
      ...
      <div>
        ...
        <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="inline-flex w-full overflow-hidden rounded-lg border border-slate-300 bg-white shadow-sm sm:w-auto">
            <button
              type="button"
      ...
      <div className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:grid-cols-2">
        <label className="flex items-start gap-3 text-sm text-slate-700 ">
          ...
          />
          <span>
            <strong className="block text-slate-900 ">Đăng ẩn danh</strong>
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
        ...
        <div className="space-y-4 rounded-2xl border border-dashed border-slate-300 bg-slate-100 p-4 ">
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
        <button
          type="button"
          onClick={() => router.back()}
          className="w-full sm:w-auto px-4 py-2 text-gray-700 hover:text-gray-900"
        >
          Hủy
        </button>
        <button
          type="submit"
          disabled={loading}
          className="w-full sm:w-auto px-6 py-2.5 rounded-xl font-semibold text-white shadow-lg hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
